...
function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      // Standard simulated Google OAuth sign-in flow
      const googleEmail = email.trim() || "demo@vistalk.org";
      try {
        await login(googleEmail, "password123");
      } catch {
        // If demo user isn't matching, create or sign in with Google session
        await signup("Google Learner", googleEmail || `learner_${Date.now()}@gmail.com`, "google_oauth_pass");
      }
      navigate(redirectPath);
    } catch (err) {
  ...
  return (
    <main className="signup-split-page login-split-page">
      {/* ====================================================
      ...
      <header className="signup-top-bar">
        ...
        {/* Top-Right Theme Toggle & Signup Link */}
        <div className="signup-top-actions">
          ...
          <div className="login-switch-text">
            ...
            <Link to="/signup" className="login-direct-link">
              Create one <span className="arrow-inline">→</span>
            </Link>
          </div>
        </div>
      </header>
      ...
      {/* ====================================================
          TWO-COLUMN HERO & LOGIN STAGE
          ==================================================== */}
      <div className="signup-stage-container">
        {/* ----------------------------------------------------
            LEFT COLUMN: SAME VISUAL ARTWORK & WELCOME BACK
            ---------------------------------------------------- */}
        <section className="signup-left-hero" aria-label="Welcome Back to VisTalk">
          <div className="signup-hero-copy">
            <h1 className="signup-hero-heading">Welcome Back</h1>
            <p className="signup-hero-subheading">
              Continue your journey with VisTalk.<br />
              Learn. Practice. Grow.
            </p>
          </div>

          {/* Exact Same Artwork Showcase */}
          <div className="signup-illustration-frame">
            <img
              src="/signup_illustration.png"
              alt="Hands communicating in sign language - VisTalk"
        ...
        {/* ----------------------------------------------------
            RIGHT COLUMN: REFINED LOGIN FORM CARD
            ---------------------------------------------------- */}
        <section className="signup-right-panel" aria-label="Login Form">
          <article className="signup-form-card">
            {/* Form Header */}
            <div className="form-card-header">
              <span className="form-eyebrow">WELCOME BACK</span>
              <h2 className="form-heading">Sign in to VisTalk</h2>
            ...
            <form onSubmit={handleSubmit} className="signup-input-form">
              ...
              <div className="login-demo-helper">
                ...
                >
                  ⚡ Fill Demo Account (demo@vistalk.org)
                </button>
              </div>
